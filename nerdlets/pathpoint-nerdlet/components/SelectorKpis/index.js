...
import { Checkbox, Fade, Well } from "react-bootstrap";
class index extends Component {
    ...
    componentDidMount() {
        this.setState({ kpis: this.props.listKpis })
    ...
    validateQuantityChecks(kpis,check){
        let quantity=0;
        const quantityChecked = kpis.filter(kpiObj => kpiObj.query!==''&&kpiObj.check);
    ...
    render() {
        ...
        return (
            <div ref={this.myRef}>
                <div style={{ height: '90%', display: 'flex', alignItems: 'center',cursor:'pointer' }} onClick={this.handleMenu}>
                    <Icon />
                </div>
                        ...
                        <div className="listKpis__content">
                            {filterKpis.map((kpi, index) =>
                                <div key={index} className='listKpis__content--item'>
                                    <Checkbox checked={kpi.check} onChange={() => this.handleCheckKpi(kpi)} />
                                    <span>{kpi.name}</span>
                                </div>
                            )}
                        </div>
...
const Icon = () =>
    <svg width="5" height="22" viewBox="0 0 5 27" fill="none" xmlns="http://www.w3.org/2000/svg">
