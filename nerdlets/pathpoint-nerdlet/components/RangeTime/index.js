...
class index extends Component {
    ...
    handleOption = (index,option) => {
        this.setState(prevState => ({ visible: !prevState.visible, selected: index }));
                ...
                {visible &&
                    <div className="options">
                        {options.map((option, index) =>
                            <div key={index} className={index === timeRangeKpi.index ? 'options--optionselected' : 'options--option'} onClick={() => this.handleOption(index,option)}>
                                {option.label}
                            </div>
                        )}
                    </div>
