...
export default class AlertIssues {
  ...
  SetGQL(accountId, cursor, resetTimeWindow) {
  ...
  async NerdGraphGetIssues() {
    ...
    for (const account of this.MeasureAccountIds) {
      const issuesList = [];
      let cursor = '';
      ...
      while (cursor !== null) {
        ...
        if (data && data.actor) {
          ...
          // console.log('Issues-Count:', data.actor.account.aiIssues.issues.issues.length);
        ...
        if (cursor) {
          ...
          gql = this.SetGQL(account.id, cursor, resetTimeWindow);
          // console.log('GQL:', gql);
        }
      }
      issuesByAccount.push({
        account,
        issuesList
      });
    };
  ...
  FilterIssuesByAccount(issuesByAccount) {
    const newList = [];
    issuesByAccount.forEach(item => {
      const conditionIds = item.account.conditionIds;
      const filterIssues = [];
      item.issuesList.forEach(issue => {
        if (this.FindID(issue.conditionFamilyId, conditionIds)) {
          if (filterIssues.length < 5) {
      ...
      newList.push({
        ...
        issuesList: filterIssues
      });
    });
    return newList;
  }

  FindID(familyId, conditionIds) {
    for (let x = 0; x < familyId.length; x++) {
      if (conditionIds.includes(familyId[x])) {
        return true;
      }
    }
    return false;
  ...
  async GetAdditionalDetailsByAccount(filterIssuesByAccount) {
    const completedInfo = [];
    for (const issue of filterIssuesByAccount) {
      const fullInfo = await this.GetAdditionalDetails(
        issue.issuesList,
        issue.account.id
      );
      completedInfo.push({
  ...
  async GetAdditionalDetails(issuesList, accountId) {
  ...
  GetTouchpointByAlertId(accountId, alertId) {
    let result = null;
    this.AlertTouchpoints.some(tp => {
      let found = false;
      tp.measure_points[0].alertConditionId.some(id => {
        let found2 = false;
        if (id === alertId && accountId === tp.measure_points[0].accountID) {
          found2 = true;
          result = tp;
          found = true;
        }
  ...
  FindAlertTrigger(accountId, issue, stages) {
    issue.conditionFamilyId.some(alertId => {
      let found = false;
      const touchpoint = this.GetTouchpointByAlertId(accountId, alertId);
      if (touchpoint) {
        found = true;
  ...
  CheckIssues(stages, measures) {
    measures.forEach(measure => {
      measure.issuesList.forEach(issue => {
        stages = this.FindAlertTrigger(measure.account.id, issue, stages);
      });
    });
    return stages;
  }
  ...
  SetRedAllAlertsTouchpoints(stages) {
    this.AlertTouchpoints.forEach(tp => {
      stages = this.SetStatus(
        stages,
        tp.stage_index,
  ...
  async Measure(stages, touchpoints, alertsTimeWindow, alertsRefreshDelay) {
    ...
    // stages = this.SetRedAllAlertsTouchpoints(stages);
  ...
  GetMeasuresAccountIds(AlertTouchpoints) {
    const accounts = [];
    AlertTouchpoints.forEach(tp => {
      if (tp.measure_points) {
        const allReadyExist = accounts.find(
          item => item.id === tp.measure_points[0].accountID
        );
        if (allReadyExist) {
          allReadyExist.conditionIds = [
            ...allReadyExist.conditionIds,
            ...tp.measure_points[0].alertConditionId
          ];
        } else {
          accounts.push({
            id: tp.measure_points[0].accountID,
            conditionIds: [...tp.measure_points[0].alertConditionId]
          });
        }
      }
    });
