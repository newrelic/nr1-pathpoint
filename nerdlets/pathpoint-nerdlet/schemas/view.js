...
export const CustomSchemaValidation = (target) => {
   ...
   target.stages.map((stage, i) => {
      stage.touchpoints.map((touchpoint, c) => {
         ...
         touchpoint.queries.map((query, x) => {
   ...
   if (currentVersion[0] !== targetVersion[0] || currentVersion[1] !== targetVersion[1]) {
      errors.push({
...
// VALIDAR QUE NO TENGA COMA, NI ESPACIO EN BLANCO ( A-Z 1-9 -_ )